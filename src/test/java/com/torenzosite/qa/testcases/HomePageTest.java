...
public class HomePageTest extends TestBase {
	...
	@Test(priority = 2, description = " verify Email Addree Field")
	public void verifyEmailAddreeField() throws IOException {
		...
		Assert.assertEquals(title, "Thank You! - Detroit, Ann Arbor, Warren | Torenzo",
				"After entering valid Email ID IT's not navigating Succefull page");

	}

	@Test(priority = 3, description = " verify Email Addree Field with email id")
	public void verifyWithEmptyEmailAddreeField() throws IOException, InterruptedException {
		...
		Assert.assertEquals(tryTorenzoForFreePage.getTextFromValidationMessage(), "This field is required.",
				"Alert Message is missing with empty email id");
		Assert.assertEquals(tryTorenzoForFreePage.getTextFromAlertMessage(),
				"There was a problem with your submission. Errors have been highlighted below.",
				"Validation message is missing with empty email id");
	...
	public void verifyWithInvalidEmailAddreeField() throws IOException, InterruptedException {
		...
		System.out.println("AlertMessage with empty email id ==>" + tryTorenzoForFreePage.getTextFromAlertMessage());
		System.out.println("validation message ==>" + tryTorenzoForFreePage.getTextFromValidationMessage());
		Assert.assertEquals(tryTorenzoForFreePage.getTextFromValidationMessage(), "Please enter a valid email address.",
				"Alert Message is missing with invalid email id");
		Assert.assertEquals(tryTorenzoForFreePage.getTextFromAlertMessage(),
				"There was a problem with your submission. Errors have been highlighted below.",
				"Validation message is missing with inavalid email id");
